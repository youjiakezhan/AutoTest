...
class Log(object):
    ...
    def catch_log(self):
        b32.configure(text='日志抓取中', state=DISABLED)
        adb_list_old = []
        adb_list_new = []
    ...
    def stop_log(self):
        b32.configure(text='抓取日志', state=NORMAL)
...
class StartTimeDialog(Toplevel):
    ...
    def setup_UI(self):
        ...
        btn1 = Button(frm3, text="取消", width='14', height='1', bg='green', fg='gold', activebackground='sky blue', command=self.cancel)
        btn1.grid(row=0, column=0, padx=5, pady=5, sticky=W)
        btn2 = Button(frm3, text="确定", width='14', height='1', bg='green', fg='gold', activebackground='sky blue', command=self.ok)
